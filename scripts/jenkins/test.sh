...
# * the lehub droplet should be deleted in "Post-build Actions".
...
IP=$(ssh "$DOCKERHOSTUSER@$DOCKERHOST" \
  "./digitalocean/scripts/new-droplet.sh lehub")
ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no \
  root@"$IP" \
  "git clone https://github.com/lehub-association/site-web.git && \
  cd site-web && \
  ./scripts/ci.sh"
